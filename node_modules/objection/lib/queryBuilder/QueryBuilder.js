...
const {
  NaiveEagerOperation,
} = require('./operations/eager/NaiveEagerOperation')
const {
  WhereInEagerOperation,
} = require('./operations/eager/WhereInEagerOperation')

const {
  InsertGraphAndFetchOperation,
} = require('./operations/InsertGraphAndFetchOperation')
const {
  UpsertGraphAndFetchOperation,
} = require('./operations/UpsertGraphAndFetchOperation')
const {
  InsertAndFetchOperation,
} = require('./operations/InsertAndFetchOperation')
const {
  UpdateAndFetchOperation,
} = require('./operations/UpdateAndFetchOperation')
...
class QueryBuilder extends QueryBuilderBase {
  ...
  isExecutable() {
    return (
      !this.isExplicitlyResolvedOrRejected() &&
      !findQueryExecutorOperation(this)
    )
  }
  ...
  joinEager(exp, modifiers) {
    return this.eagerAlgorithm(getJoinEagerAlgorithm(this)).eager(
      exp,
      modifiers,
    )
  }
  ...
  naiveEager(exp, modifiers) {
    return this.eagerAlgorithm(getNaiveEagerAlgorithm(this)).eager(
      exp,
      modifiers,
    )
  }
  ...
  mergeEager(exp, modifiers) {
    ...
    eagerOp.expression = eagerOp.expression.merge(
  ...
  mergeJoinEager(exp, modifiers) {
    return this.eagerAlgorithm(this.modelClass().JoinEagerAlgorithm).mergeEager(
      exp,
      modifiers,
    )
  }
  ...
  mergeNaiveEager(exp, modifiers) {
    return this.eagerAlgorithm(
      this.modelClass().NaiveEagerAlgorithm,
    ).mergeEager(exp, modifiers)
  }
  ...
  mergeAllowEager(exp) {
    ...
    if (eagerOp.allowedExpression) {
      ...
      eagerOp.allowedExpression = parseRelationExpression(
        this.modelClass(),
        exp,
      )
    }
  ...
  findOptions(opt) {
    if (arguments.length !== 0) {
      this._findOperationOptions = Object.assign(
        {},
      ...
      if (op.constructor === SelectOperation) {
        ...
        return (
          ...
          op.selections.every(select => {
            return (
              (!select.table || select.table === tableRef) &&
              select.column === '*'
            )
          })
        )
  ...
  insertAndFetch(modelsOrObjects) {
    return writeOperation(this, () => {
      ...
      const insertAndFetchOperation = new InsertAndFetchOperation(
        'insertAndFetch',
        {
  ...
  insertGraphAndFetch(modelsOrObjects, opt) {
    return writeOperation(this, () => {
      ...
      const insertGraphAndFetchOperation = new InsertGraphAndFetchOperation(
        'insertGraphAndFetch',
        {
  ...
  updateAndFetch(modelOrObject) {
    return writeOperation(this, () => {
      ...
      if (!(updateOperation.instance instanceof this.modelClass())) {
        throw new Error(
          'updateAndFetch can only be called for instance operations',
        )
      }
      ...
      this.addOperation(updateAndFetch, [
        updateOperation.instance.$id(),
        modelOrObject,
      ])
    })
  }
  ...
  upsertGraphAndFetch(modelsOrObjects, upsertOptions) {
    return writeOperation(this, () => {
      ...
      const upsertGraphAndFetchOperation = new UpsertGraphAndFetchOperation(
        'upsertGraphAndFetch',
        {
  ...
  patchAndFetch(modelOrObject) {
    return writeOperation(this, () => {
      ...
      if (!(patchOperation.instance instanceof this.modelClass())) {
        throw new Error(
          'patchAndFetch can only be called for instance operations',
        )
      }
      ...
      this.addOperation(patchAndFetch, [
        patchOperation.instance.$id(),
        modelOrObject,
      ])
    })
  }
...
function checkEager(builder) {
  ...
  if (
    expr.numChildren > 0 &&
    allowedExpr &&
    !allowedExpr.isSubExpression(expr)
  ) {
...
function buildKnexQuery(builder, knexBuilder = builder.knex().queryBuilder()) {
  ...
  if (
    !builder.isPartial() &&
    !hasSelects &&
    (!fromOperation || fromOperation.table)
  ) {
