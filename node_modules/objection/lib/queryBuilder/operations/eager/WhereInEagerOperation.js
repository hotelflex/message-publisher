...
} = require('../../../model/RelationDoesNotExistError')
...
class WhereInEagerOperation extends EagerOperation {
  ...
  onBuild(builder) {
    ...
    const { selectionsToAdd, selectedProps } = findRelationPropsToSelect(
      builder,
      relationsToFetch,
    )
  ...
  onAfter2(builder, result) {
    ...
    const promise = promiseUtils.map(
      this.relationsToFetch,
      it =>
        this.fetchRelation(builder, models, it.relation, it.childExpression),
      ...
      try {
        ...
        if (err instanceof modelClass.ModifierNotFoundError) {
          throw modelClass.createValidationError({
            ...
            message: `could not find modifier "${modifierName}" for relation "${
              relation.name
            }"`,
          })
        } else {
...
function findRelationsToFetch(builder, eagerExpression) {
  ...
  try {
    eagerExpression.forEachChildExpression(
      modelClass,
      (childExpression, relation) => {
...
function findRelationPropsToSelect(builder, relationsToFetch) {
  ...
  for (const { relation } of relationsToFetch) {
    ...
    for (let c = 0, lc = ownerProp.size; c < lc; ++c) {
      ...
      if (
        !builder.hasSelectionAs(fullCol, col) &&
        selectionsToAdd.indexOf(fullCol) === -1
      ) {
